...
Rocket::Rocket()
    ...
    , m_hasExploded(false)
{
    SPARK_ASSERT_MSG(m_explosionRadius > 0.0f, "Rocket explosion radius must be positive");
    ...
    SPARK_ASSERT_MSG(scale.x > 0 && scale.y > 0 && scale.z > 0, "Rocket scale must be positive");
    SetScale(scale);
}

HRESULT Rocket::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
    SPARK_ASSERT_MSG(device != nullptr, "Rocket::Initialize device is null");
    SPARK_ASSERT_MSG(context != nullptr, "Rocket::Initialize context is null");

    HRESULT hr = Projectile::Initialize(device, context);
    SPARK_ASSERT_MSG(SUCCEEDED(hr), "Projectile::Initialize failed in Rocket");
    return hr;
}

void Rocket::Update(float deltaTime)
{
    SPARK_ASSERT_MSG(deltaTime >= 0.0f && std::isfinite(deltaTime), "Invalid deltaTime in Rocket::Update");
...
{
    SPARK_ASSERT_MSG(m_mesh != nullptr, "Rocket mesh not initialized");
    Projectile::Render(view, projection);
}

void Rocket::OnHit(GameObject* target)
{
    SPARK_ASSERT_MSG(target != nullptr, "Rocket::OnHit target is null");
    if (!m_hasExploded)
        Explode(GetPosition());
}
...
{
    SPARK_ASSERT_MSG(!m_hasExploded, "Rocket exploded multiple times");
    m_hasExploded = true;
