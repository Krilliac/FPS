...
HRESULT Game::Initialize(GraphicsEngine* graphics,
    InputManager* input)
{
    SPARK_ASSERT(graphics != nullptr);
    SPARK_ASSERT(input != nullptr);
    ...
    SPARK_ASSERT(m_shader);

    HRESULT hr = m_shader->Initialize(
        m_graphics->GetDevice(),
        m_graphics->GetContext());
    SPARK_ASSERT_MSG(SUCCEEDED(hr), "Shader::Initialize failed");
    if (FAILED(hr)) return hr;

    hr = m_shader->LoadVertexShader(L"Shaders\\HLSL\\BasicVS.hlsl");
    SPARK_ASSERT_MSG(SUCCEEDED(hr), "LoadVertexShader failed");
    if (FAILED(hr)) return hr;

    hr = m_shader->LoadPixelShader(L"Shaders\\HLSL\\BasicPS.hlsl");
    SPARK_ASSERT_MSG(SUCCEEDED(hr), "LoadPixelShader failed");
    if (FAILED(hr)) return hr;

    /* Player -----------------------------------------------*/
    m_player = std::make_unique<Player>();
    SPARK_ASSERT(m_player);
    ...
    hr = m_player->Initialize(
        ...
        m_camera.get(),
        m_input);
    SPARK_ASSERT_MSG(SUCCEEDED(hr), "Player::Initialize failed");
    if (FAILED(hr)) return hr;

    /* Projectile pool --------------------------------------*/
    m_projectilePool = std::make_unique<ProjectilePool>(100);
    SPARK_ASSERT(m_projectilePool);
